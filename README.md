...
>   the code generated by the Twig 3.27+ compiler.
> - Support for anonymous [symfony/ux-twig-component](https://symfony.com/bundles/ux-twig-component/current/index.html)
>   components (`<twig:x>`), as requested in [#230](https://github.com/twigstan/twigstan/issues/230):
>   props and their defaults, detection of missing required props, `attributes`, embedded bodies.
>   The body of a component is analyzed with the context of its usage site, including `{% set %}`
>   variables assigned earlier in the enclosing block.
> - Support for Symfony form themes: the blocks of a template whose `{% use %}` or `{% extends %}`
>   chain reaches a built-in theme of symfony/twig-bridge are analyzed with the context that
...
>
> These changes are meant to be proposed upstream if the project becomes active again.
